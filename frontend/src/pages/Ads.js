...
function Ads() {
    ...
    const [assignedFilter, setaassignedFilter] = useState("")
    ...
    var head = "Advertisement"
        ...
        useEffect(() => {
            ...
            async function fetchData() {
                 ...
                 if (newFilter.length) {
                    const data = await  axios.get(`http://127.0.0.1:5000/api/products/admin/pending?new=${'new'}`)
                    const res1 = data.data;  
                    const filter1 = res1.filter((newVal1) => {
                        //Search Filtering
                        return newVal1.name.startsWith(newFilter) 
                      });
    
                      setNewProduct(filter1);
                } else {
                 ...
                 if (assignedFilter.length) {
                    const data = await  axios.get(`http://127.0.0.1:5000/api/products/admin/pending?assigned=${'assigned'}`)
                    const res1 = data.data;  
                    const filter1 = res1.filter((newVal1) => {
                        //Search Filtering
                        return newVal1.name.startsWith(assignedFilter) 
                      });
    
                      setAssignedProduct(filter1);
                } else {

                    const data = await axios.get(`http://127.0.0.1:5000/api/products/admin/pending?assigned=${'assigned'}`)
                    console.log(data.data)
                    setAssignedProduct(data.data)                    
                }
                ...
                if(pendingFilter.length){
                  const data1 = await axios.get(`http://127.0.0.1:5000/api/products/admin/pending?pending=${'pending'}`)
          ...
          }, [pendingFilter,approvedFilter,newFilter,assignedFilter])
          ...
          const displayResult4 = assignedProduct
              ...
              .map((product) => {
              return <AdList title={product.name} id={product._id} date={format(product.createdAt)} product={product} allChecked={assignedFilter}/>;
              });
          ...
          const displayResult2 =  pendingProduct
              ...
              .map((product) => {
                if(product.name.startsWith(pendingFilter)){
                    ...
                    return <AdList title={product.name} id={product._id} date={format(product.createdAt)} product={product} allChecked={pendingallChecked}/>;
                }
              });
                ...
                <Headings heading={head}/>
                    ...
                    <div class="tab-content" id="pills-tabContent">
                               ...
                               <div class="search-area d-flex p-4">
                                <input class="form-control me-3" placeholder="Search Ad" onChange={(e) => (setnewFilter(e.target.value))}/>
                            ...
                            <div class="search-area d-flex p-4">
                                <input class="form-control me-3" placeholder="Search Ad" onChange={(e) => (setpendingFilter(e.target.value))}/>
                                <button class="btn btn-primary">Search</button>
                            </div>
                                <div class="table-responsive">
                                    <table class="table">
                            ...
                            <div class="tab-pane fade" id="pills-pending" role="tabpanel" aria-labelledby="pills-pending-tab">
                                <div class="search-area d-flex p-4">
                                    <input class="form-control me-3" placeholder="Search Ad"  onChange={(e) => (setaassignedFilter(e.target.value)) }   />
                                    <button class="btn btn-primary">Search</button>
                                </div>
                                <div class="table-responsive">
                                    <table class="table">
                            ...
                            <div class="tab-pane fade" id="pills-approved" role="tabpanel" aria-labelledby="pills-approved-tab">
                                ...
                                <div class="search-area d-flex p-4">
                                    <input class="form-control me-3" placeholder="Search Ad"  onChange={(e) => (setapprovedFilter(e.target.value))} />
                                    <button class="btn btn-primary">Search</button>
                                </div>
